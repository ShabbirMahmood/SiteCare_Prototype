...
3. `storage.initialize()` creates directories/tables if needed, enables WAL, checks the schema version, initializes ID counters, and retries committed photo deletions.
...
| `app.js` | Hash routes for overview, patients, appointments, audit, settings, help and `patient/{id}`. Mounts/disposes the photo workspace and coordinates shared clock refresh. |
...
| `clock-ui.js` | Displays the effective date and offers the administrator's system/manual clock form. |
...
- `schema_info.version` is currently `1`. Startup creates missing declared tables and rejects unsupported schema versions. There is no general versioned migration framework for arbitrary future schema changes.
...
The diagram shows authenticated sessions; a bootstrap session may have no user yet. `audit.patient_id` deliberately has **no foreign key**, so audit records survive patient deletion. `app_clock`, `schema_info`, `id_sequences`, and `pending_photo_deletions` are installation-level tables outside the patient graph.
...
| `sites` | Composite key `(patient_id, number)`; `x`, `y` | The 14 persistent numbered positions in centimetres. |
| `photos` | `id`, `patient_id`; unique `filename`; dimensions, capture/upload times, `alignment_json`, verification fields, `locked`, `demo` | Photo metadata and saved geometry. Image data itself stays on disk. |
...
Patient/time indexes support event and complication lookup. JSON columns hold alignment snapshots, warning lists, observation types and variable audit details. Core relationships and searchable dates remain ordinary SQL columns.

`site_number` in events and complications is assigned by application logic; it is not a composite foreign key to `sites`. Geometry freezes after the first such record, preserving the interpretation of historical numbers.
...
5. Store a generated filename and unverified initial alignment, bump the patient version, and audit the upload. Handled transaction failures remove the newly written file.
...
| Photo readiness | Current screening needs a calibrated, verified photograph captured between now minus 24 hours and now. |
...
1. Screen all 14 stored centres.
...
Custom layout saves validate exactly one of each number 1–14, coordinate bounds, 5 cm navel distance, and 2.5 cm separation between numbered centres. The API rejects changes once any puncture or skin record exists, even if subsequently voided or recovered.

**Default Layout** submits `reset_to_default: true` with the current patient version to the same layout endpoint. The server calls `default_sites()`, applies the same validation/record lock, saves all positions, bumps the version, and audits `layout.reset_to_default` with before/after coordinates. A request cannot specify both reset and custom sites.
...
sequenceDiagram
    ...
    API->>DB: Begin write; check duplicate key and patient version
    ...
    alt Procedure passes checks or valid historical documentation
        ...
        API->>DB: Lock photo; update appointment, version and audit
        API->>DB: Commit
...
An alert stores a centre, radius, observation types, severity, observed/recorded times, note, actor and photo-alignment snapshot. Its radius is 0.3–15 cm. Marking an alert requires the latest current, calibrated photo and locks that photo's alignment.
...
This is an as-of calculation over current stored records, **not a complete historical snapshot** of every profile, layout or alignment revision. Advancing time can make photos stale; it never automatically heals skin alerts.

## 10. API Organization

All paths below are relative to the local server. Patient mutations generally submit `version`; browser writes carry CSRF and clock-revision headers. Photo upload uses multipart form data; other writes use JSON.

| Routes | Access | Purpose |
| --- | --- | --- |
| `GET /api/bootstrap` | Anonymous or signed in | Setup-needed flag, session/CSRF, user, policy and time |
...
| `POST /api/patients/{id}/layout` | Signed in | Save custom or default positions before records lock the layout |
| `POST /api/patients/{id}/screen-point` | Signed in | Screen an exact point without recording a puncture |
...
`ui.js` centralizes API calls, CSRF and clock headers, auth-loss handling, HTML escaping, dates, dialogs and toasts. Background reads use an activity flag so polling does not keep an idle login alive. Language preference and cross-tab clock-change signals use local storage; patient records and passwords do not use it as persistent storage.
...
Sessions are removed from the **backup copy**, not the live database. The ZIP contains `data/sitecare.sqlite3`, referenced `data/photos/` files, and `RESTORE.txt`. It includes accounts/password hashes, patients, audit history and clock configuration. CSV export includes patient event/skin rows and is not a full backup.
...
The fixtures create temporary databases, test accounts and generated images rather than operating on the user's installation. The latest recorded application run before this documentation update completed with **103 tests passed**, with one existing Starlette/AnyIO deprecation warning. See [Test Report](docs/TEST_REPORT.md) for dates and scope. That result does not represent clinical validation or exhaustive browser testing.
